...
def plot_function(function_input: str, have_solution: bool, root: float) -> None:
    ...
    # Convierte el string a una expresión simbólica. Esto permite empezar a trabajar con la función como un objeto matemático en lugar de un string.
    func = sp.sympify(function_input)

    # Convertimos la función a una función lambda para que pueda ser interpretada como una función de python, a la cual se le manda un array de valores como parametro y devuelve otro array con los valores de la función evaluada en cada uno de los valores del array de entrada.
    ...
    # Evaluamos la función en el rango de valores de 'x' que generamos anteriormente.
    y_vals = func_lambda(x_vals)

    # Se grafica la función con el conjunto de x's y y's que generamos anteriormente.
